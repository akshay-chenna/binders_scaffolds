...
def main(
    ...
    restraint_content = Path(restraint_path).read_text()

    results = chai1_inference.remote(fasta_content, inference_config, run_id, restraint_content)
...
image = modal.Image.debian_slim(python_version="3.12").run_commands(
    "pip install chai_lab==0.5.0 hf_transfer==0.1.8"
)
...
def chai1_inference(
    fasta_content: str, inference_config: dict, run_id: str, restraint_content: str,
) -> list[(bytes, str)]:
    from pathlib import Path
    ...
    restraint_file = Path("/tmp/restraint.content")
    restraint_file.write_text(restraint_content.strip())
